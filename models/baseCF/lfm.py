...
from models.lib.utils import LogTime
...
class LFM:
    ...
    def __init__(self, model_dir: str, K=100, epochs=10, alpha=0.01, lamb=0.01, n_rec_movie=10, save_model=True):
        """
    ...
    def init_users_items_set(self, trainset):
        ...
        for user, movies in trainset.items():
            self.user_average_score[user] = sum(movies.values()) / len(movies)
            for item, rate in movies.items():
                total_rate = self.item_average_score.get(item, 0.0)
                total_rate += rate
                self.item_average_score[item] = total_rate
                item_popular[item] += 1
                users_set.add(user)
                items_set.add(item)
                items_list.append(item)
        for item, popular in item_popular.items():
            self.item_average_score[item] /= popular
    ...
    def predict(self, user, item):
        ...
        if user not in self.P:
            print('user not in train-set: %s' % str(user))
            item_average_score = self.item_average_score.get(item, 2.5)
            return item_average_score
        if item not in self.Q:
            print('item not in train-set: %s' % str(item))
            user_average_score = self.user_average_score[user]
    ...
    def prediction(self, testset: list):
        ...
        """
        predictions = list(map(lambda x: self.predict(x[0], x[1]), testset))
        return predictions
    ...
    def fit(self, trainset, retrain=True):
        ...
        if not retrain:
            self.P = model_manager.load_pkl('P')
            self.Q = model_manager.load_pkl('Q')
            print('model loaded')
        else:
            ...
            if self.save_model:
                model_manager.save_pkl(self.P, 'P')
                model_manager.save_pkl(self.Q, 'Q')
