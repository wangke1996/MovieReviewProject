...
class DoubanCrawler:
    def __init__(self):
        self.key = '0b2bdeda43b5688921839c8ecb20399b'
        self.url_pre = 'https://api.douban.com/v2/movie'
    ...
    def get_object_item(self, route_path, cache_file_name, update_interval=-1):
        """
        ...
        url = '%s/%s?apikey=%s' % (self.url_pre, route_path, self.key)
        return self.get_json_data(json_file, url, update_interval=update_interval)

    def get_list_items(self, route_path, field_name, require_count=-1, update_interval=-1):
        """
        ...
        while require_count < 0 or start < require_count:
            ...
            url = '%s/%s?apikey=%s&start=%d&count=%d' % (self.url_pre, route_path, self.key, start, count)
    ...
    def get_movie_top250(self, movie_count=250, update_interval=720):
        """
        爬取最受欢迎的电影
        :param movie_count: 爬取电影数量
        :param update_interval: 本地缓存的生命周期（小时），-1代表永久有效
    ...
    def get_celebrity_info(self, celebrity_id, update_interval=720):
        """
        爬取影人信息
